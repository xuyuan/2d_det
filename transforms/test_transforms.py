...
class Preprocessor(object):
    ...
    def __call__(self, pil_image, min_dim=None, data_aug=None):
        ...
        batch = self.pad_divisible(batch, height_width, min_dim)
        return batch, height_width

    def pad_divisible(self, batch, height_width, min_dim=None):
        if min_dim is None:
...
class TestTransform(object):
    def __init__(self, min_dim):
        self.preprocessor = Preprocessor(min_dim)
        self.batch_squeezed = False

    def pre_process(self, data, min_dim, data_aug=None):
        self.batch_squeezed = False
        if isinstance(data, Image):
            ...
            if data.dim() > 4:
                data.squeeze_(dim=0)
            data = self.preprocessor.pad_divisible(data, height_width, min_dim)
    ...
    def post_process(self, height_width, y, conf_thresh, nms_thresh, max_bbox=0):
    ...
    def post_process_mask(self, height_width, mask, bboxes, conf_thresh, nms_thresh, mask_thresh, mask2rbox, data_aug=None):
    ...
    def post_process_mask_with_elevation(self, height_width, mask, elevation, bboxes, conf_thresh, nms_thresh, data_aug=None):
        mask = F.softmax(mask, dim=1)
        if data_aug:
            mask = self.decode_augmented_mask(mask, data_aug)

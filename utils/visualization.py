...
def draw_bbox_on_image(image, bboxes, classnames=None, relative=False, line_width=None, font=None):
    ...
    if relative:
        scale = np.ones(bboxes.shape[1])
        scale[0] = scale[2] = width
        scale[1] = scale[3] = height
        bboxes = np.asarray(bboxes) * scale

    if line_width is None:
        line_width = min(3, int(min(width, height) / 640)) + 1
    ...
    for bbox in bboxes:
        ...
        t = str(bbox[-1])
        if classnames:
            draw.text((bbox[0:2] + line_width).tolist(), classnames[label] + t, fill=color, font=font)
        else:
            bbox_size = (bbox[2:4] - bbox[:2]).astype(np.int)
            draw.text((bbox[0:2] + line_width).tolist(), f'{bbox_size}, {t}', fill=color, font=font)

    return image


def draw_train_sample(sample, classnames=None, font=None):
    """
    Args:
        sample (dict):
            input: PIL.Image
    ...
    if 'bbox' in sample:
        img = draw_bbox_on_image(img, sample['bbox'], classnames=classnames, relative=True, font=font)
    if 'masks' in sample:
...
def draw_detection(image, detection, classnames, scale=None, prob_threshold=0, image_threshold=0, seen_classes_number=0):
    ...
    for i, (name, det) in enumerate(zip(classnames, bboxes)):
        ...
        if seen_classes_number > 0:
            color = LABEL_COLOR[int(cls > seen_classes_number)]
        else:
            color = LABEL_COLOR[cls % len(LABEL_COLOR)]
        for d in det:
            prob, box = d[0], d[1:]

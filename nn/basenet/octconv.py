#  based on https://github.com/d-li14/octconv.pytorch
...
model_urls = {
    'oct_resnet50': 'https://drontheimerstr.synology.me/model_zoo/oct_resnet50_cosine-3bcff007.pth',
    'oct_resnet101': 'https://drontheimerstr.synology.me/model_zoo/oct_resnet101_cosine-138f29ed.pth',
    'oct_resnet152': 'https://drontheimerstr.synology.me/model_zoo/oct_resnet152_cosine-386f6fa3.pth',
}
...
class Conv_BN(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, alpha_in=0.5, alpha_out=0.5, stride=1, padding=0, dilation=1,
        ...
        self.out_channels = int(out_channels * (1 - alpha_out))
...
def oct_resnet26(pretrained=False, **kwargs):
    ...
    if pretrained:
        raise NotImplementedError('pretrained oct_resnet26')
    return model
...
def oct_resnet50(pretrained=False, **kwargs):
    ...
    if pretrained:
        state_dict = load_state_dict_from_url(model_urls['oct_resnet50'])
...
def oct_resnet101(pretrained=False, **kwargs):
    ...
    if pretrained:
        state_dict = load_state_dict_from_url(model_urls['oct_resnet101'])
        model.load_state_dict(state_dict)
    return model
...
def oct_resnet152(pretrained=False, **kwargs):
    ...
    if pretrained:
        state_dict = load_state_dict_from_url(model_urls['oct_resnet152'])
        model.load_state_dict(state_dict)
    return model
...
def oct_resnet200(pretrained=False, **kwargs):
    ...
    if pretrained:
        raise NotImplementedError('pretrained oct_resnet200')
    return model

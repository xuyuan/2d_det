...
class Tester(object):
    def __init__(self, create_model=torch.load, device='auto', jobs=1, disable_tqdm=None, cudnn_benchmark=False,
        ...
        self.num_dataloader_workers = num_dataloader_workers or get_num_workers(-self.jobs) // self.jobs  # (#cpus - #jobs) / (#jobs)
    ...
    def inference(self, model_file, dataset, output_root=None, process_id=None, cuda_device_id=None, use_half=False, sub_processes=1, batch_size=0):
        model = self._create_or_load_model(model_file, cuda_device_id, use_half, sub_processes)
        predict = getattr(model, 'predict', model)
        det, inference_time = inference(predict, dataset, output_root, process_id, self.disable_tqdm, sub_processes=sub_processes,
    ...
    def test(self, model_file, dataset, output=None, data_balanced=False, sub_processes=1, batch_size=0, resume=None):
        ...
        if num_processes == 1:
            ...
            r = self.inference(model_file, dataset, output_root, cuda_device_id=cuda_device_id, use_half=use_half, sub_processes=sub_processes, batch_size=batch_size)
            results.append(r)
        else:
        ...
        for r in results:
            p = r['prediction']
            inference_time.append(r['inference_time'])

            if isinstance(p, str):
                detection_files.append(p)
            elif isinstance(p, list):
                detection_files += p
            else:
                detections.update(p)

        info = {'inference_time': max(inference_time),
                'model': results[0]['model']}
        predictions = Predictions([detections] + detection_files, info=info, mode='w')
        if resume:

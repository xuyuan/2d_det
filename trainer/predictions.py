...
class Predictions(object):
    def __init__(self, predictions, info={}, mode='r'):
        """
    ...
    def save(self, filename):
        state_dict = dict(predictions=self.predictions, info=self.info)
        pickle.dump(state_dict, open(filename, 'wb'))

    @staticmethod
    def open(filename, mode='r', n_new_file=0):
        if mode == 'r':
            ...
            p = Predictions(predictions, mode='w')
            p.save(filename)
            return p
        else:
